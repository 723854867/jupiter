...
import org.jupiter.redis.ILuaCmd;
import org.jupiter.redis.LuaCmd;
...
@Slf4j
@Component
public class LuaOps extends Ops {
	...
	private void init() throws Exception {
		...
		try {
			...
			Resource[] resources = ConfigLoader.RESOLVER.getResources("classpath*:conf/lua/*.lua");
			for (Resource resource : resources) {
				...
				bin.read(buffer);
				_addScript(key.toLowerCase(), buffer);
			}
	...
	public void setBitRange(String key, long start, long stop, boolean value) {
		Assert.isTrue(start < stop, "start must less than stop");
	 ...
	 */
	public long captchaObtain(String captchaKey, String countKey, String captcha, int lifeTime, int countMaxinum, int countLifetTime, int interval) {
		return execute(LuaCmd.CAPTCHA_OBTAIN, captchaKey, countKey, captcha, lifeTime, countMaxinum, countLifetTime, interval);
	}
