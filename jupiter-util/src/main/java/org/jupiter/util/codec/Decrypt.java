...
public class Decrypt {
	...
	public static String AESDecode(String aesKey, String content) {
		try {
			...
			return new String(buffer, JupiterConsts.UTF_8);
		} catch (Exception e) {
	...
	public static boolean RSASignVerify(String text, byte[] sign, String publicKey, SignatureAlgorithm algorithm) {
		try {
			...
			signature.update(text.getBytes(JupiterConsts.UTF_8));
	 ...
	 * @param privateKey
	 */
	public static byte[] RSADecodeByPrivateKey(byte[] encryptData, String privateKey, CryptTransformation transformation) {
		try {
